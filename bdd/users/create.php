<?php
    ...
    if (empty(($_POST["nickname"]))) {
        header('location: /roleAttribution.php?created=2');
    } else {
        ...
        if ($connexion->prepare($checkNickname)) {
            ...
            if (!empty($foundNickname)) {
                header('location: /roleAttribution.php?created=3');
            } else {
                // Validate password
                if (empty($_POST["password"])) {
                    header('location: /roleAttribution.php?created=4');
                } else {
                    // Validate confirm password
                    if (empty($_POST["confirmPassword"])) {
                        header('location: /roleAttribution.php?created=5');
                    } else if (($_POST ['password'] != $_POST['confirmPassword'])) {
                        header('location: /roleAttribution.php?created=6');
                    } else {
                        if (empty($_POST['email'])) {
                            header('location: /roleAttribution.php?created=7');
                        } else {
                            ...
                            $createUser->execute();
                            header('location: /roleAttribution.php?created=1');
                        }
                    }
                }
            }
        } else {
            echo("<script> alert('Oops! Un erreur est survenue! Veuillez réessayer plus tard.'); window.location.href=roroleAttribution.phption.php; </script>");
        }
    }
